...
# -----------------------------
df = pd.read_csv(
    "20251016 140537-ILLumiSense-Strain-CH8.txt",
    sep="\t",
    header=38,
    encoding="cp1252"
)
...
# -----------------------------
fill_volumes = np.array([0, 246.89, 244.46, 242.15, 245.21, 244.49, 243.69, 246.88, 245.15])/(10**8)
fill_times = [df["Time"][0], "14:06:00", "14:07:00", "14:08:00", "14:09:00", "14:10:00", "14:11:00", "14:12:00", "14:13:00"]
...
def plot_channel(channel_name):
    ...
    ax1.plot(df["Time"], df[channel_name], color="blue", label=f"Experimental ({channel_name})")
    ax1.plot(df["Time"], anticipated_strains * 1e7, color="green", linestyle="dashed", label="Expected")
    ax1.set_xlabel("Time")
    ...
    fig.autofmt_xdate()
    fig.suptitle(f"Experimental vs Expected Microstrain — {channel_name}", fontsize=20)
    ax1.grid(True)
